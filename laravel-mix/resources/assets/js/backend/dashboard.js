...
$(document).ready(function() {
    ...
    function createChart(ele_param, eleTooltip_param, color_param){
        let ctx;
        let result;
        let ele = ele_param;
        ...
        let customTooltips = function(tooltip) {
            ...
            if (tooltip.dataPoints.length > 0) {
                ...
                let val_number;
                if(type === 'engagement'){
                    val_number = `${formatNumber(val)} vistors`;
                }else if(type === 'conversion'){
                    val_number = `${formatNumber(val)} buyers`;
                }else{
                    val_number = `$${formatNumber(val)}`;
                }
    ...
    function runChart(valChart, paramVisitor = 7){
        ...
        createChart('statistic-sale', 'tooltip-sale', {gradient:'rgba(0, 157, 206, 0.7)', border: 'rgb(0, 157, 206)'}).init(valChart.sales, paramVisitor, 'sales');
    }
    ...
    function  getStatisticDay(visitor = 7){
        $.ajax({
            ...
            success:function(data) {
                ...
                $('span#statistic_visitor_sales').empty().html('$'+data.sales);
