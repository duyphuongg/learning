document.addEventListener("DOMContentLoaded", function(event) {
  ...
  // Handle click on menu responsive
  handleSidebarMenyResponsive();
});
...
function aliSlideOwlStep() {
    ...
    var owl = $('#owl-carousel-get-start');
...
function multiSelectBootstrap(nonText) {
  $('.multiselect').multiselect({
    nonSelectedText: nonText || 'All ratings',
    templates: { 
...
function handleSidebarMenyResponsive(){
  if(checkMobileApple()){
    $('.sidebar-menu > li').on('mouseover', function(){
      if($(this).hasClass('alrv-toggle-intercom')){
        let intercom_on_messager = document.querySelector('.intercom-messenger-frame');
        if( intercom_on_messager == null ) {
            Intercom('show');
        } else {
            Intercom('hide');
      ...
      let is_url_exist = /javascript/i.test(url);
      if(is_url_exist){
        return;
      }
      location.href = url;
...
function checkMobileApple(){
  let is_apple = /iPhone|iPad|iPod/i.test(navigator.userAgent);
  if (is_apple) {
    return true;
  }
  return false;
