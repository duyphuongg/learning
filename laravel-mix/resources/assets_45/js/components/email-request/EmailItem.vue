        ...
        <form :data-vv-scope="'form-2-'+  email_item.id">
            ...
            <div class="mb-25">
                ...
                <div class="row d-flex align-items-center" style="min-height: 35px;">
                    ...
                    <div class="col-md-4">
                        <label :for="'email_logo' + email_item.id" class="email-file-upload-label color-dark-blue button button--default fw600" style="width: 100%">
                            Browse Image
                        </label>
                        <input type="file" name="email_banner" :id="'email_logo' + email_item.id" class="hidden" @click="$event.target.value = null" @change="onLogoChange">
                    </div>
                </div>
            </div>
            ...
            <div class="mb-20" v-if="email_item_type_prop === 'email-request'">
                <input type="text" v-validate="'required'" :class="{ 'is-danger': errors.has('form-2-'+email_item.id+'.review_box') }" class="form-control" name="review_box" placeholder="Write your review" v-model="email_item.review_box">
                <small v-show="errors.has('form-2-'+email_item.id+'.review_box')" class="field-text is-danger mt-5">{{ errors.first('form-2-'+email_item.id+'.review_box') }}</small>
            </div>
            ...
            <div class="row mb-20">
                <div class="col-md-8">
                    ...
                    <input type="text" name="button_text" v-validate="'required'" :class="{ 'is-danger': errors.has('form-2-'+email_item.id+'.button_text') }" class="form-control" placeholder="Review Now" v-model="email_item.button_text">
                    <small v-show="errors.has('form-2-'+email_item.id+'.button_text')" class="field-text is-danger mt-5">{{ errors.first('form-2-'+email_item.id+'.button_text') }}</small>
                </div>
            ...
            <div class="mb-25" v-if="email_item_type_prop !== 'email-request'">
                ...
                <input type="text" name="button_link" v-validate="'required'" :class="{ 'is-danger': errors.has('form-2-'+email_item.id+'.button_link') }" class="form-control" placeholder="https://your-domain.com/" v-model="email_item.button_link">
                <small v-show="errors.has('form-2-'+email_item.id+'.button_link')" class="field-text is-danger mt-5">{{ errors.first('form-2-'+email_item.id+'.button_link') }}</small>
            </div>

            <!--  -->
            <div class="mt-20 dropdown">
                <button type="button" class="button button--primary mr-5 fw600 dropdown-toggle btn--orange" data-toggle="dropdown" :disabled="listen_on_change_data || !email_settings.next_test">Send A Test Email</button>
                <span style="position: relative; top: 0px; color: #f94c4e; font-weight: 600;" v-if="!email_settings.next_test">*You can send 24 test emails per day only</span>
                <span style="position: relative; top: 0px; color: #f94c4e; font-weight: 600;" v-else-if="listen_on_change_data">Save your settings before sending test email.</span>
                <div class="email-item__send-email-test__popup dropdown-menu">
                    ...
                    <div class="input-group">
                        ...
                        <span class="input-group-btn">
                            <button class="btn button button--primary fw600 btn--orange" type="button" @click="sendEmailTest()" :disabled="is_disabled">Send</button>
                        </span>
                    </div>
                </div>
            </div>
