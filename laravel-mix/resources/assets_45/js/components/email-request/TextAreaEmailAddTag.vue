    ...
    <div :class="class_name" class="js-wrap-email-add-tag">
        <label class="fz15 color-dark-blue fw600 mt-5 d-flex">
            ...
            <div class="dropdown">
                ...
                <i class="fas alireview-fa-folder" data-toggle="dropdown"></i>
        ...
        <div contenteditable="true" :class="{ 'is-danger': !data_prop }" class="alrv-textarea-add-tag js-alrv-textarea-add-tag" :placeholder="placeholder_prop" v-html="renderTagToHTML(data, html_tags_email)"
             v-on:keydown="onInputEmailTag($event)"
             v-on:blur="onBlurContentEditAble()"
             v-on:focusout="onFocusOutInputEmail($event)"
             @paste="onPaste($event)"
        ></div>
        <small v-show="!data_prop" class="field-text is-danger mt-5">This field is required!</small>
    </div>
</template>
