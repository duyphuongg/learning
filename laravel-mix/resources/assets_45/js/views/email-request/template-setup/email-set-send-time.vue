        ...
        <div class="email-set-send-time__target-customer">
            <div class="target-customer__content">
                ...
                <ul class="email-target-list">
                    ...
                    <li>
                        <button class="button btn-add-country-timing" @click="addTarget()"><i class="far alireview-fa-plus"></i> Add Country Timing</button>
                    </li>
                </ul>
            </div>
            <div class="target-customer__footer">
                    <h4>Send Review Request Email</h4>
            </div>
        ...
        <div class="row email-set-send-time__content">
            <div class="col-md-6 email-set-send-time__list" v-if="schedule_email_customs.length > 0">
                <email-time
                    ...
                    v-bind:valid_email_time_prop="schedule_email_customs.length > 0 && email_customs_prop.length <= 0"
                    @onRemoveEmailTime="onRemoveEmailTime($event)"
                    @onAddCustomEmail="onAddCustomEmail($event)"
                ></email-time>
            </div>
            <div class="col-md-6 email-set-send-time__list email-disabled" v-else>
                <email-time
                        v-bind:index_prop="0"
    ...
    export default {
        ...
        methods: {
            ...
            setSelectIndexEmailTime: function(){
                this.email_custom_preview = (this.schedule_email_customs[this.select_item_index] && this.schedule_email_customs[this.select_item_index].email_custom) ?this.email_customs[this.schedule_email_customs[this.select_item_index].email_custom] : email_custom_setting;
            },
            checkValidEmailTime: function(){
                if(this.schedule_email_customs.length > 0 && this.email_customs_prop.length <= 0){
                    ...
                    if(this.schedule_email_customs.length > 0){
                        console.log("this.email_customs", this.email_customs, this.schedule_email_customs, this.select_item_index)
