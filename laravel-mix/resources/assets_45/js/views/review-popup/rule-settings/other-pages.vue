<template>
    <div class="rp__rule-other-pages">
        <div class="other-pages">
            <div class="row mar-b-30 mar-t-10">
                ...
                <div class="col-xs-12 mar-b-25">
                    <input-radio class_name="" label="Randomize all product reviews" name="other_conditions_products" :value="page_rule_settings_default.other.products_type[0]" :selected_value="settings.products_type" @change="onConditions" />
                </div>
                <div class="col-xs-12">
                    <input-radio class_name="" label="Only show reviews of specific products" name="other_conditions_products" :value="page_rule_settings_default.other.products_type[1]" :selected_value="settings.products_type" @change="onConditions" />
                </div>
            </div>
            <search-and-select-products 
                v-if="list_select_products_specific"
                :disabled="settings.products_type !== page_rule_settings_default.other.products_type[1]"
                :list_products_prop="list_products_prop"
                :list_selected_prop="list_select_products_specific"
                :is_show_valid="settings.products_type == page_rule_settings_default.other.products_type[1]"
    ...
    export default {
        ...
        methods: {
            getProductsSpecific: async function(){
                let res = await getProductsSpecific('other');
                console.log("getProductsSpecific other",res)
                if(res.status){
